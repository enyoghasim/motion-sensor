...
def get_latest_motion(device_id: str, service: MotionService = Depends(get_motion_service)):
    return service.get_latest_motion(device_id)
...
def get_motion_history(
    ...
    return service.get_motion_history(device_id, limit)


@router.post("/report")
def report_motion(event: schemas.MotionReport, service: MotionService = Depends(get_motion_service)):
    service.report_motion(event.device_id, event.motion_detected)
    return {
        "status": "recorded",
